...
result_fields = [
    "model_name", "provider", "model", "id", "category", "question", "context", "expected", "actual_output", "answer_relevancy_score", "answer_relevancy_passed", "answer_relevancy_reason", "factual_correctness_score", "factual_correctness_passed", "factual_correctness_reason", "error",
]
...
def call_model( provider: str, model: str, question: str ) -> str:
    ...
    response = client.chat.completions.create( model = model, temperature = 0, messages = [{ "role": "system", "content": "Atbildi latviešu valodā. Sniedz īsu, precīzu un faktoloģiski korektu atbildi.", }, { "role": "user", "content": question },], )
    return response.choices[0].message.content.strip()

def create_metrics( ):
    answer_relevancy = AnswerRelevancyMetric( threshold = 0.7 )
    factual_correctness = GEval( name = "Factual Correctness", criteria = "Evaluate whether the actual output correctly answers the input question and is consistent with the expected output. Penalize hallucinated or unsupported claims.", evaluation_params = [eval_params.INPUT, eval_params.ACTUAL_OUTPUT, eval_params.EXPECTED_OUTPUT,], threshold=0.7, )
...
def evaluate_row(model_config: dict, row: dict) -> dict:
    ...
    test_case = LLMTestCase( input = row["question"], actual_output = actual_output, expected_output = row["expected"], )
